...
Description: 
	Go into Firefox history; list visited sites and the
...
WARNING:
Currently not accurate, as the places.mysql file holds History AS WELL AS other things (like Bookmarks).
=cut
...
#open Firefox History file; get the lines; close the file
opendir DIR, "$dir/AppData/Roaming/Mozilla/Firefox/Profiles/" or die "File doesn't exist!\n";
my $places;
while (my $file = readdir(DIR)) {
	if ($file =~ m/(.*)default/) {
	$places = "$dir/AppData/Roaming/Mozilla/Firefox/Profiles/$file/places.sqlite";
	}
}
closedir DIR;
open FILE, $places or die $!;
my @lines = <FILE>;
close FILE;
...
my $workbook  = Spreadsheet::WriteExcel->new( 'FirefoxResult.xls' );
